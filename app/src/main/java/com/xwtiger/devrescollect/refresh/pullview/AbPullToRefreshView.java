 ...
 */
public class AbPullToRefreshView extends LinearLayout implements OnScrollListener{
	...
	protected void onFinishInflate() {
		...
		{
			GridView gv = (GridView)mAdapterView;
	...
	private boolean isRefreshViewScroll(int deltaY) {
		...
		if (mAdapterView != null) {
			...
			if (deltaY > 0) {
				...
				Log.d("test", "getTop=="+mAdapterView.getTop());
				//替换上面注掉的代码
				int top2 = child.getTop();
				float x2 = child.getY();
				int paddingTop = mAdapterView.getPaddingTop();
				int top3 = mAdapterView.getTop();
				
				
				Log.d("test", "child.getTop()=="+child.getTop());
				Log.d("test", "child.getY()=="+child.getY());
				Log.d("test", "mAdapterView.getPaddingTop()=="+mAdapterView.getPaddingTop());
				Log.d("test", "mAdapterView.getTop()=="+mAdapterView.getTop());
				...
				if (mAdapterView.getFirstVisiblePosition() == 0) {
					...
					if(r.top ==0)
					{
						...
						Log.d("test", "r.top=="+r.top);
						Log.d("test", "r.right=="+r.right);
						Log.d("test", "r.bottom=="+r.bottom);
						return true;
					}
				}
				...
				Log.d("test", "Bottom=="+lastChild.getBottom());
				Log.d("test", "getHeight=="+getHeight());
				Log.d("test", "getBottom=="+mAdapterView.getBottom());
				/*if (lastChild.getBottom() <= getHeight() && mAdapterView.getLastVisiblePosition() == mAdapterView.getCount() - 1) {
					mPullState = PULL_UP_STATE;
					return true;
				}*/
				//替换上面注掉的代码
				if ( mAdapterView.getLastVisiblePosition() == mAdapterView.getCount() - 1) {
					
					Rect r = new Rect();
					lastChild.getLocalVisibleRect(r);
					Log.d("testbottom","r.top="+ r.top);
					Log.d("testbottom","r.right="+ r.right);
					Log.d("testbottom","r.bottom="+ r.bottom);
					
					if(r.bottom==lastChild.getHeight())
					{
						mPullState = PULL_UP_STATE;
						return true;
					}
	...
	public void onHeaderRefreshFinish() {
		...
		if(mHeaderView.getState() ==AbListViewHeader.STATE_NORMAL)
		{
			return;
		}
		//setHeaderTopMargin(-mHeaderViewHeight);
		mScroller.startScroll(0, mHeaderViewHeight, 0, -mHeaderViewHeight, SCROLL_DURATION);
		mHeaderView.setState(AbListViewHeader.STATE_NORMAL); 
		if(mAdapterView!=null){
	...
	public void computeScroll() {
		if(mScroller.computeScrollOffset())
		{
			...
			setHeaderTopMargin(mScroller.getCurrY()-mHeaderViewHeight);
			//postInvalidate();
		}
		super.computeScroll();
	}
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
            if (mEnableLoadMore&& mAdapterView != null && mAdapterView.getLastVisiblePosition() == mAdapterView.getCount() - 1) {
            	mFooterView.setState(AbListViewFooter.STATE_LOADING);
            	footerLoading();
            	//Log.d("testpullto", "=="+ mAdapterView.getLastVisiblePosition());
            }
        }
	}

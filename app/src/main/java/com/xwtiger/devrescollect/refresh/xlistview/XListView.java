...
//    private static final String TAG = "XListView";
        ...
        if (null != observer) {
            observer.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    mHeaderHeight = mHeaderContent.getMeasuredHeight();//getHeight
    ...
    public void loadOver() {
        mFooterView.setState(XFooterView.STATE_OVER);
        mPullLoading = false;
        mFooterView.setOnClickListener(null);
    }
    ...
    public void autoRefresh() {
        mHeader.setVisibleHeight(mHeaderHeight);//mHeaderHeight
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (mEnablePullLoad && !mPullLoading) {
            if (height > PULL_LOAD_MORE_DELTA) {
            	...
            	//2016-11.28添加STATE_OVER判断
            	if(mFooterView.getmState()== XFooterView.STATE_OVER)
            	{
            		 mFooterView.setState(XFooterView.STATE_OVER);
            	}else{
            		 mFooterView.setState(XFooterView.STATE_NORMAL);
        ...
        switch (ev.getAction()) {
            ...
            default:
                ...
                if (getFirstVisiblePosition() == 0) {
                    // invoke refresh
                    if (mEnablePullRefresh && mHeader.getVisibleHeight() > mHeaderHeight&&(!mPullRefreshing)&&(!mPullLoading)) {
                        mPullRefreshing = true;
                        mHeader.setState(XHeaderView.STATE_REFRESHING);
                        refresh();
                        Log.d("testrefresh", "mPullRefresh");
                    }else{
                    	mPullRefreshing = false;
                    	mHeader.setState(XHeaderView.STATE_NORMAL);
                    ...
                    if (mEnablePullLoad && mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA&&(!mPullLoading)&&(!mPullRefreshing)) {
                        startLoadMore();
                        Log.d("testrefresh", "mPullLoading");
                    }else{
                    	 mPullLoading = false;
                    	 if(mFooterView.getmState() ==XFooterView.STATE_OVER)
                    	 {
                    	 ...
                    	 Log.d("testrefresh", "mPullLoading -----no");
                    }
                    resetFooterHeight();
                }
                break;
...
//        }
    ...
    public XHeaderView getHeadView()
    {
    	return mHeader;
    }
